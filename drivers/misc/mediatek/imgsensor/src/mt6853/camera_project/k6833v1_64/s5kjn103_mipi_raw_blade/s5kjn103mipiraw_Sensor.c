...
#define PFX "blade s5kjn103_camera_sensor"
#define LOG_1 LOG_INF("blade s5kjn103,MIPI 4LANE\n")
/****************************   Modify end	**************************/
...
#define DEVICE_VERSION_S5KJN103	 "s5kjn103_mipi_raw_blade"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5KJN103_SENSOR_ID_BLADE,
...
static kal_int32 write_Module_data(ACDK_SENSOR_ENGMODE_STEREO_STRUCT * pStereodata)
{
	...
	if(pStereodata != NULL) {
		...
		if ((pStereodata->uSensorId == S5KJN103_SENSOR_ID_BLADE) && (data_length == DUALCAM_CALI_DATA_LENGTH)
			...
			if (*sensor_id == S5KJN103_SENSOR_ID) {
				*sensor_id = imgsensor_info.sensor_id;
				...
				if(deviceInfo_register_value == 0x00){
					ret = Eeprom_DataInit(0, S5KJN103_SENSOR_ID_BLADE);
