...
/***************for enter ffc wait 20s suspended charging**************************/
int mt6370_set_for_ffc(bool suspend)
{
	if (!oplus_chg_data) {
		printk(KERN_ERR "%s NULL\n", __func__);
		return -1;
	...
	printk(KERN_ERR "%s suspend:%d suspended:%d\n", __func__, suspend, atomic_read(&oplus_chg_data->suspended));

	return 0;
}
EXPORT_SYMBOL(mt6370_set_for_ffc);
